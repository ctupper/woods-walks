...
*Describes BMAD-METHOD v6.12.0. Source: `skills/bmad-build/` (source ID P18, see `../sources.md`). Tags: [V, P18] traced to the skill's own files, [I] inferred, [?] unclear. [V-lab] marks claims observed in a lab run (Experiments 2, 3, 4b, 5h, 5i; see [lab-log.md](lab-log.md)). First written unattended 2026-09-19; lab findings added through 2026-09-24; reviewed by Carl 2026-09-24.*
...
- **Scope:** one user-facing goal, 900 to 1600 tokens. "Neither limit is a gate"; both are proposals the user can override. Multi-goal means two or more independently shippable deliverables. [V, P18] [?] re-check: the template section below gives a 900 to 1300 target with 1600 as the flag point; whether "900 to 1600" here is the workflow's own wording or a merge of the two is unverified against the tag.
...
- This is exactly the docs' "three checks: intent gaps, irreversible actions, footprint" ([flow.md](flow.md)). Experiment 2 took the dispatch path and left `route: dispatch` in its spec. [V, P18r; V-lab]
...
- **Two gaps in this mechanism, found by running a real epic through it and then retrospecting on the result (Experiment 5i):**
  - A `false` verdict has no expiry. Triage renders `false` from what is true *right now*; nothing records the condition a "no path does X" refutation depends on, so a later story that introduces path X has no prompt to revisit an earlier story's `false`. Observed: story 1 verdicted a duplicate-id finding `false` on "no path creates duplicates" (true then); story 2 re-cited that verdict by reference rather than re-deriving it; story 4 introduced the first path that deletes, and the original premise was already false by the time anyone re-checked. [V-lab, [lab-log.md](lab-log.md)]
  - `bad_spec` routing logs to `## Spec Change Log` only when observable behavior changes — a triage decision that changes a frozen bullet's *stated mechanism* while producing the same output (the exact case is documented, not what actually runs) leaves no record at all, because "changes no matrix row" reads as nothing to log. Observed: a triage row changed a frozen boundary from checking a tag after lowercasing to checking it on raw input (correctly, to reject Unicode confusables); the frozen text, the spec, and the eventual commit message all still describe the old order, three artifacts out of sync with the one comment that's right. [V-lab, [lab-log.md](lab-log.md)]
...
- **Implementation Notes can go stale the moment review patches land, with nothing to catch it (Experiment 5i).** The template says Implementation Notes is agent-owned and append-only, but nothing requires a second write after the review loop finishes. Observed across a four-story epic: two stories' Implementation Notes quoted test counts that were already wrong by the time review patched in more tests (off by 2 and by 1), while the corresponding commit *messages*, written after review, had the correct counts the whole time. The artifact meant for the next story's Code Map to read is the one surface that doesn't get a second pass. [V-lab, [lab-log.md](lab-log.md)]
- **A review pass that discovers a better test shape has no mechanism to propose it for stories already closed.** Observed: a reviewer on the fourth story of an epic invented a mutation-guard assertion (capture the pre-call array, assert it untouched after) that would have caught the same class of bug in the first two stories, neither of which had it. Verified by mutation testing during a later retrospective: rewriting an earlier story's function to mutate in place still passed its full test suite. The gap is structural, not a one-off miss — nothing in the review loop generates a backfill task across already-`done` stories when a later one finds a stronger check. [V-lab, [lab-log.md](lab-log.md)]
- **A deferred defect can end up pinned by a test as if it were intended.** In the tags epic, the `nextId` id-reuse bug was deferred and also covered by a test asserting the reissue happens, with no marker saying "this documents a known defect, not the contract." Whoever fixes the bug must first find and invert that test. The deferral mechanism (`deferred-work.md`) and the test suite do not reference each other. [V-lab, [lab-log.md](lab-log.md) Experiment 5i, finding A7]
- **A testable spec line can be asserted nowhere, and no review layer notices across stories.** `SPEC.md` said the epic introduces "three distinct new error codes." The codes appear 25 times in the tests, always as an individual expected value, never compared to each other; a refactor collapsing two of them would pass all 61 tests. The Verification Gap Reviewer runs per story, and each story introduced one code, so no single review saw the spec line as unverified. [V-lab, [lab-log.md](lab-log.md) Experiment 5i, finding B7]
...
- **The frozen-Never rule has no escape hatch for cross-story cleanup, observed to cost real findings (Experiment 5i).** A story's Never clause bars touching any earlier story's code; three consecutive stories in one epic independently found the same duplication and rejected fixing it for that exact reason. Each rejection was locally correct — extracting a shared helper really would have edited a prior story's frozen code — and the aggregate result was duplication every reviewer saw and none could act on. Nothing in the workflow schedules a cross-story refactor slice at the end of a spec to absorb exactly this kind of finding. [V-lab, [lab-log.md](lab-log.md)]
