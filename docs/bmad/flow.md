...
- **`bmad-retrospective`** reads the epic's evidence (specs, story records, full diff, commits, tracking) and produces a review, action items, and a verdict: `accepted`, `accepted-with-open-items`, or `rejected`. Unfinished stories make it `rejected`. Every finding needs a source reference. It proposes; nothing touches code or specs automatically. [V, P9]
  - **Lab note (Experiment 5i):** run on a finished four-story epic, its own `bmad-review` pass over the *combined* diff found the epic's most serious defect — a live-object aliasing bug reachable through the public API, silent, defeating validation rules the code otherwise enforced. No individual story's review had found it, because each story's mutation was locally consistent with what came before; only stacking all four diffs together showed the compound effect. The retrospective also traced a `nextId` bug to a `false` triage verdict from story 1 that story 2 re-cited without re-deriving, later falsified by story 4 — a finding about how a correct decision goes stale, not about the code. 16 action items came out of it, six of them about gaps in BMAD's own process (a checkpoint with no completion condition, a `false` verdict with no expiry, no backfill mechanism when a later story finds a better test, and others) rather than about the software. [V-lab, [lab-log.md](lab-log.md)]
...
- ~~What does a real `SPEC.md` and `stories.yaml` look like?~~ Experiments 5 to 5h produced and evolved both through a full epic; see [spec-skill.md](spec-skill.md). (Story 4's spec went over the 1600-token flag; Carl accepted.) [V-lab]
- ~~Where does a human actually get pulled in during a `bmad-build` run?~~ At the checkpoints the step files name (dirty tree, subagent permission, Open Questions, approval, token count), every one of which fired as documented across Experiment 5h; see [build-step-by-step.md](build-step-by-step.md). [V-lab]

One lab observation so far: `bmad-help` on a tiny idea recommended skipping the planning pipeline and going straight to `bmad-spec` then `bmad-build`, consistent with the sizing rule above. [V-lab, see [lab-log.md](lab-log.md)]
